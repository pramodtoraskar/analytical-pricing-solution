...
class ExcelToHdf5(object):
    ...
    def convert_xldate_to_timestamp(self, xldate, data_book):
        ...
        print type(date)
        print date
        print type(date.timetuple())
        print date.timetuple()
        print "===================================\n\n"
        #return time.mktime(date.timetuple())

        con_np_datetime = np.datetime64(date)

        print con_np_datetime
        print type(con_np_datetime)

        return_value = (con_np_datetime.astype('uint64') / 1e6).astype('uint32')
